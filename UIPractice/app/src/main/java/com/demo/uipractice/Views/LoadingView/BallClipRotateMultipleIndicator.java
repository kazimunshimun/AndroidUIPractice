package com.demo.uipractice.Views.LoadingView;
...
public class BallClipRotateMultipleIndicator extends BaseIndicatorController{
    ...
    public void draw(Canvas canvas, Paint paint) {
        ...
        for (int i = 0; i < 1; i++) {
         ...
         //   canvas.drawArc(rectF, bStartAngles[i], 150, false, paint);
        }
    ...
    //    float[] sStartAngles=new float[]{225,45};
        ...
        for (int i = 0; i < 1; i++) {
            ...
            canvas.drawArc(rectF, sStartAngles[i], 150, false, paint);
        }
        ...
        canvas.scale(scaleFloat, scaleFloat);
        canvas.rotate(-degrees);
        //draw two smallest arc
        //    float[] sStartAngles=new float[]{225,45};
        float[] smStartAngles=new float[]{135,-45};
        for (int i = 0; i < 1; i++) {
            paint.setColor(Color.BLUE);
            RectF rectF=new RectF(-x/3.6f+circleSpacing,-y/3.6f+circleSpacing,x/3.6f-circleSpacing,y/3.6f-circleSpacing);
            canvas.drawArc(rectF, smStartAngles[i], 150, false, paint);
        }
